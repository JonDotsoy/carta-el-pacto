...
const carta_images_astro_file = new URL("../carta-images.astro", import.meta.url)
...
for (const product of carta_payload) {
    ...
    if (product.nombre === "SPICY TEQUILA")
        console.log("🚀 ~ id:", id)
    let image_file: URL | null = null

    const product_image_element = dump_payload(`[alt=${JSON.stringify(product.nombre)}]`)

    const source_target_image = product_image_element.prop('src')
    const isDefaultImage = source_target_image?.endsWith("product-default.svg") ?? false
